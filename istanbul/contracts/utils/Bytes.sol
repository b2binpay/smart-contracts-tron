...
library Bytes {
    ...
    function slice(bytes memory buffer, uint256 start, uint256 end) internal pure returns (bytes memory) {
        ...
        assembly ("memory-safe") {
            ...
            // tail < 32 bytes
            if gt(len, 0) {
                // mask = 2^(8*(32-len)) - 1  (mask for highest bytes, which cannot be overwritten)
                let mask := sub(exp(256, sub(32, len)), 1)
                // dest is already zero (new bytes), so and(mload(dest), mask) can be omitted, so it's for compatibility
